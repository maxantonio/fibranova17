...
// jQuery(document).ready(function($) {
    $.ajax({
        url: 'http://marktdaten.irstrat.com/historicos/670.json',
        async: false,
        dataType: 'jsonp',
...
//  $('#table-time').html(intradia["time"]);
    ...
    $('#table-volume').html(numberWithCommas(intradia["volume"]));
    $('#price-ticker').html("$" + intradia["price"]);
    $('#change-ticker').html(intradia["change"]);

    $('#price-ticker-title').html(' $' + intradia["price"]);
    $('#volumen-ticker').html(' ' + numberWithCommas(intradia["volume"]));
